...
export default function DecisionHistory() {
  const { decisions } = useDemo()
  const accepted = decisions.filter((d) => d.status === 'accepted').length
  const totalROI = decisions.filter((d) => d.roi).reduce((s, d) => s + d.roi, 0)
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
        <KpiCard title="Total Decisions" value={decisions.length} icon={MessageSquare} format="number" delay={0} />
            ...
            {decisions.map((decision, i) => {
              ...
              return (
                <motion.div
                  ...
                  transition={{ delay: decision.isNew ? 0 : i * 0.08, type: 'spring', stiffness: 300 }}
                ...
                >
                  ...
                  <Card className={decision.isNew ? 'ring-2 ring-primary/30 shadow-lg shadow-primary/10' : ''}>
                    {decision.isNew && (
                      <motion.span
                        initial={{ opacity: 0, y: -4 }}
                        animate={{ opacity: 1, y: 0 }}
                        className="inline-block mb-2 px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider rounded-full bg-primary/10 text-primary"
                      >
                        New — from simulation
                      </motion.span>
                    )}
